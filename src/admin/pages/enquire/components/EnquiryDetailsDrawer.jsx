...
export default function EnquiryDetailsDrawer({ enquiry, open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      />
      <aside
        className={`absolute right-0 top-0 h-full w-full max-w-full overflow-y-auto border-l border-white/15 bg-[#0f2e1a]/98 p-5 backdrop-blur-xl transition-transform duration-300 ease-out sm:w-[75%] sm:max-w-[75%] md:p-6 ${
      ...
      >
        ...
        <div className="grid gap-3 rounded-2xl border border-white/15 bg-white/8 p-4 text-sm text-white/90 sm:grid-cols-2">
          <div>
          ...
          <div>
            ...
            <p className="mt-1 font-medium text-white">{text(enquiry?.phoneNumber)}</p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-white/70">
              Email
            </p>
            <p className="mt-1 font-medium text-white">{text(enquiry?.email)}</p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-white/70">
              Gender / Age
        ...
        <div className="mt-5">
          <h3 className="text-base font-semibold text-white">Answers</h3>
          <div className="mt-3 space-y-3">
            {(enquiry?.answers || []).map((a, idx) => (
              ...
              >
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  <span className="rounded-full border border-[#5eead4]/35 bg-[#5eead4]/10 px-2.5 py-1 font-semibold text-[#a7f3d0]">
                    {text(a?.value)}
                  </span>
                </div>
              </div>
            ...
            {Array.isArray(enquiry?.answers) && enquiry.answers.length === 0 ? (
              <div className="rounded-2xl border border-white/15 bg-white/8 p-4 text-sm text-white/85">
                No answers found.
              </div>
            ) : null}
          </div>
